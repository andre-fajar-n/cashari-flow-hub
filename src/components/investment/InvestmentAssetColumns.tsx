      ...
      return (
        <div className="flex flex-col">
          <span className="font-medium text-gray-900">
            {formatAmountCurrency(averagePrice, assetSummary.currencyCode)}
          </span>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-col">
          <span className="font-semibold text-gray-900">
            {formatAmountCurrency(currentValue, assetSummary.currencyCode)}
          </span>
          <span className="text-xs text-gray-500">
            {assetSummary.totalAmountUnit.toLocaleString('id-ID')} unit × {formatAmountCurrency(assetSummary.latestAssetValue, assetSummary.currencyCode)}
          </span>
        </div>
      );
    },
      ...
      return (
        <div className={`flex items-center gap-1 ${bgClass} px-2 py-1 rounded-lg w-fit`}>
          ...
          <span className={`font-semibold text-sm ${colorClass}`}>
            {formatAmountCurrency(Math.abs(assetSummary.unrealizedAmount), assetSummary.currencyCode)}
          </span>
