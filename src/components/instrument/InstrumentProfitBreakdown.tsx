...
interface InstrumentProfitBreakdownProps {
  ...
  return (
    <div className="flex justify-between items-start py-3 border-b last:border-b-0">
      ...
      <div className="text-right">
        {isMultiCurrency ? (
          <AmountText amount={baseValue} showSign={true} className="text-sm font-semibold">
            {formatAmountCurrency(Math.abs(baseValue), baseCurrency, baseCurrency)}
          </AmountText>
        ) : (
          <>
...
const InstrumentProfitBreakdown = ({ summary }: InstrumentProfitBreakdownProps) => {
          ...
          <CardContent className="pt-0">
            ...
            <div className="py-3 border-b">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  <AmountText
                    amount={summary.unrealizedProfitBaseCurrency}
                    showSign={true}
                    className="text-sm font-semibold"
                  >
                    {formatAmountCurrency(Math.abs(summary.unrealizedProfitBaseCurrency), summary.baseCurrencyCode, summary.baseCurrencyCode)}
                  </AmountText>
                  {!summary.isMultiCurrency && summary.originalCurrencyCode !== summary.baseCurrencyCode && (
                    <p className="text-xs text-muted-foreground italic">
                      {summary.originalCurrencyCode}: {formatAmountCurrency(summary.unrealizedProfit, summary.originalCurrencyCode, summary.originalCurrencyCode)}
                    </p>
                  )}
                </div>
              </div>
...
export default InstrumentProfitBreakdown;