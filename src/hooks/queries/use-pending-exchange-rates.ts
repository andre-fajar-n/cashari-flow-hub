...
export const usePendingExchangeRates = () => {
  const query = useQuery({
    ...
    queryFn: async (): Promise<PendingPair[]> => {
      ...
      const currencyPairs = await fetchAllRows<{ currency_code: string; base_currency_code: string }>(
        supabase.from('currency_pairs').select('currency_code, base_currency_code')
          .order('currency_code').order('base_currency_code') as any
      );
      ...
      const todayRates = await fetchAllRows<{ from_currency: string; to_currency: string }>(
        supabase.from('exchange_rates').select('from_currency, to_currency')
          .eq('date', today).order('id', { ascending: true }) as any
      );
